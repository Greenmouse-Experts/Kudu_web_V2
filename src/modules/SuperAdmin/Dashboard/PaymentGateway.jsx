...
const PaymentGateway = ({ data, refetch }) => {
    ...
    const paymentGateway = (formData, id) => {
        mutate({
            url: "/admin/payment-gateway",
            method: id ? "PUT" : "POST",
            data: id ? { ...formData, id } : formData,
            headers: true,
            onSuccess: (response) => {
                closeModal();
                refetch();
    ...
    const newGateway = (data) => {
        if (data) {
            ...
            setValue('name', null);
            setValue('publicKey', null);
            setValue('secretKey', null);
        }

        openModal({
            size: "lg",
            content: <>
                <form className="grid grid-cols-2 gap-6" onSubmit={handleSubmit((formData) => paymentGateway(formData, data ? data.id : null))}>
                    <div>
                        <label htmlFor="Name" className="block text-sm font-medium mb-2 mt-4">
                            Payment Gateway
                        </label>
                        <input
                            type="text"
                            ...
                            placeholder="Enter payment gateway e.g Paystack, Flutterwave"
                            style={{ outline: "none" }}
                            {...register("name", { required: "Payment Gateway is required" })}
                        />
                        {errors.name && (
                    ...
                    <div>
                        ...
                        />
                        {errors.publicKey && (
                            <p className="text-red-500 text-sm">{errors.publicKey.message}</p>
                        )}
                    </div>
                    <div>
                        <label htmlFor="Name" className="block text-sm font-medium mb-2 mt-2">
                            Secret Key
                        </label>
                        <input
                            type="text"
    ...
    const handleDeleteModal = (id) => {
        openModal({
            size: "sm",
            content: <DeleteModal title={'Do you wish to delete this gateway?'} redirect={handleRedirect}
                api={`/admin/payment-gateway?id=${id}`} />
        })
    }
    ...
    const activate = (id) => {
        mutate({
            url: `/admin/payment-gateways/status?id=${id}`,
            method: "PATCH",
            headers: true,
            onSuccess: (response) => {
                refetch();
                ...
                <div className="w-full flex h-auto flex-col px-3 py-6 gap-3 -mt-3">
                    <div className="flex gap-5 justify-center w-full">
                        <p className="font-semibold text-center text-lg">
                            Do you wish to activate this payment gateway
                        </p>
                    </div>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className=" text-black-600 text-md font-medium">
                                    ...
                                    <th className="py-6 px-4 text-left">Public Key</th>
                                    <th className="py-6 px-4 text-left">Secret Key</th>
                            ...
                            <tbody>
                                {data.map((gateway, index) => (
                                    ...
                                    >
                                        ...
                                        <td className="py-6 px-4 text-left capitalize">{gateway.name}</td>
                                        <td className="py-6 px-4 text-left">{'********'}</td>
                                        <td className="py-6 px-4 text-left">{'********'}</td>
                                        <td className={`py-6 px-4 text-left ${gateway.isActive ? 'text-green-500' : 'text-red-500'}`}>{gateway.isActive ? 'Active' : 'Inactive'}</td>
                                            ...
                                            <Menu placement="left">
                                                ...
                                                <MenuList>
                                                    <MenuItem className="flex flex-col gap-3">
                                                        <span className="cursor-pointer w-full" onClick={() => newGateway(gateway)}>
                                                            Edit
                                                        </span>
                                                    </MenuItem>
                                                    <MenuItem className="flex flex-col gap-3">
                                                        <span className="cursor-pointer w-full" onClick={() => activateGateway(gateway.id)}>
                                                            Activate
                                                        </span>
                                                    </MenuItem>
                                                    <MenuItem className="flex flex-col gap-3">
                                                        <span className="cursor-pointer w-full" onClick={() => handleDeleteModal(gateway.id)}>
                                                            Delete
                                                        </span>
                                                    </MenuItem>
                                                </MenuList>
